...
typedef enum : uint8_t {
  ...
  PINCER_ATTACK_STATE       = 0xF7   // Leader attacks target simultaneously with Follower
} State;
...
// Manager for Pincer Attack State
struct PincerAttackManager {
  float    sel_heading;      // absolute heading toward target
  uint16_t sel_distance_cm;  // distance to travel
  bool          has_init = false;
  unsigned long start_ms;
  double        count_cm = 0;
} pincer_attack_manager;
...
void loop() {
  ...
  switch (cur_state) {
    ...
    case State::COMPASS_CALIBRATION_STATE: {
      ...
      if ((cur_timestamp - calibrate_compass_manager.start_time_ms) < calibrate_compass_manager.sel_duration_ms) {
        ...
        // Set setpoints every iteration — spin in place
        // Left forward, Right backward → clockwise rotation
        motor_left_setpoint  = DEFAULT_CALIB_RPM;
        motor_right_setpoint = DEFAULT_CALIB_RPM;
        motor_left_forward   = true;
    ...
    case State::PINCER_ATTACK_STATE: {

      // Leader charges toward the target — Same as Follower ATTACK_STATE
      must_correct_heading = true;

      double tmp_diff = double(pincer_attack_manager.sel_heading) - double(cur_heading);
      if ((abs(tmp_diff) > 180) && (tmp_diff < 0)) tmp_diff += 360.0;
      else if ((abs(tmp_diff) > 180) && (tmp_diff > 0)) tmp_diff -= 360.0;
      heading_input = tmp_diff;

      if (!pincer_attack_manager.has_init) {
        resetMotorLeftPID();
        resetMotorRightPID();
        resetHeadingPID();
        pincer_attack_manager.start_ms  = cur_timestamp;
        pincer_attack_manager.count_cm  = 0;
        pincer_attack_manager.has_init  = true;
      }
      ...
      double dx = 14.137166941 * double(current_speed) * double(time_diff) / 60000.0;
      pincer_attack_manager.count_cm += dx;

      if (pincer_attack_manager.count_cm >= pincer_attack_manager.sel_distance_cm) {
        pincer_attack_manager.has_init = false;
        cur_state = State::STOPPED_STATE;
        Serial.println(F("Pincer attack complete."));
      }
  ...
  // ============================================================
  //  HUSKYLENS FSM — paused during PINCER_ATTACK_STATE
  // ============================================================
  if (cur_state != State::PINCER_ATTACK_STATE) {

    // Non-blocking servo write
    static unsigned long servo_last_ms = 0;
    if (millis() - servo_last_ms >= 15) {
      huskyServo.write(servo_pos);
      servo_last_ms = millis();
    }
    ...
    switch (husky_state) {
      ...
      case HuskyState::LOCATE_FOLLOWER_STATE: {
        ...
        if (follower_obj.detected) {
          ...
          if ((!follower_left && !follower_right) || (follower_left && follower_right)) {
            Serial.print(F("\nFollower:"));
      ...
      case HuskyState::LOCATE_TARGET_STATE: {
        ...
        if (target_obj.detected) {
          ...
          if ((!target_left && !target_right) || (target_left && target_right)) {

            float angle_deg    = calcAngleDeg(target_obj.xCenter);
            float dist_cm      = calcDistanceCm(target_obj.width, target_obj.size_cm);
            float servo_offset = float(servo_pos - SERVO_ZERO_POS);

            Serial.print(F("\nTarget:"));
            Serial.print(F("\tDistance(cm): ")); Serial.print(dist_cm);
            Serial.print(F("\tAngle: "));        Serial.print(angle_deg);

            // Step 1: Send attack to Follower
            sendAttackToFollower(angle_deg, dist_cm, ATTACK_HEADING_RELATIVE_FLAG | ATTACK_DISTANCE_FLAG);

            // Step 2: Trigger Leader's own pincer attack
            executePincerAttack(angle_deg, servo_offset, dist_cm);
  ...
  } // end if not PINCER_ATTACK_STATE
}
...
// Triggers Leader's own attack toward the target
// angle_from_camera: pixel offset → degrees from calcAngleDeg()
// servo_offset: servo_pos - SERVO_ZERO_POS
// distance_cm: estimated distance to target
void executePincerAttack(float angle_from_camera, float servo_offset, float distance_cm) {

  // Convert camera angle + servo offset → absolute heading
  float absolute_heading = cur_heading + servo_offset + angle_from_camera;

  // Normalization [0, 360]
  if (absolute_heading < 0)    absolute_heading += 360.0f;
  if (absolute_heading >= 360) absolute_heading -= 360.0f;

  // Populate pincer attack manager
  pincer_attack_manager.sel_heading     = absolute_heading;
  pincer_attack_manager.sel_distance_cm = (uint16_t)distance_cm;
  pincer_attack_manager.has_init        = false;
  pincer_attack_manager.count_cm        = 0;

  // Request state change
  req_state = State::PINCER_ATTACK_STATE;

  Serial.println(F("→ Leader PINCER heading: "));
  Serial.print(absolute_heading);
  Serial.print(F("° dist: "));
  Serial.print(distance_cm);
  Serial.println(F(" cm"));
}
